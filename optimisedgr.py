...
gray=cv2.imread("c:/users/girishhegde/forestgray.jpg",0)
gray=cv2.resize(gray,(275*2,183*2),interpolation=cv2.INTER_AREA)
cv2.imshow('gray',gray)
print(gray.shape)
cv2.imwrite("gray4.jpg",gray)
base=cv2.imread("c:/users/girishhegde/fbase.jpg")
base=cv2.resize(base,(275*2,183*2),interpolation=cv2.INTER_AREA)
...
def paint(ipimg,base,rgbbase,ltrans):
	...
	for i in range(m):
		...
		for j in range(n):
			...
			for k,l in ind[ipimg[i,j]]:

							smnew=abs(sdt[k,l]-sdi[i,j])
...
ind=(indexvalue(labbase[:labbase.shape[0],:labbase.shape[1],0]))
