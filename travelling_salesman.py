...
class DNA():
    ...
    dictionary = np.array(list('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_ '))
    ...
    def create(self):
        return np.random.choice(dictionary, self.length)

    def find_fitness(self, target):
        self.fitness = np.where(self.gene == target)[0].shape[0]
        self.fitness = self.fitness[i] ** 2
